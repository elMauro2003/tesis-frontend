...
export function ViewStudentPanel({ studentId, onClose }: ViewStudentPanelProps) {
  ...
  useEffect(() => {
    ...
    const wingName = roomData.wing && typeof roomData.wing === 'object'
      ? roomData.wing.name
      : (roomData.wing ?? '')?.toString?.() || undefined;
  ...
  const locationLabel = currentRoom
    ...
    : (student?.current_room_info
      ? `${student.current_room_info.room_number || 'Sin número'}${student.current_room_info.wing ? ` · ${student.current_room_info.wing}` : ''}${student.current_room_info.building ? ` · ${student.current_room_info.building}` : ''}`
      : (student?.current_room ? `${student.current_room.number || 'Sin número'}${student.current_room.wing ? ` · ${student.current_room.wing}` : ''}${student.current_room.building ? ` · ${student.current_room.building}` : ''}` : 'Sin ubicación'));
  ...
  const truncate = (s: string | undefined | null, n = 18) => {
    if (!s) return '';
    return s.length > n ? `${s.slice(0, n - 1)}…` : s;
  };

  const careerBadge = yearValue ? `${yearValue}° Año` : truncate(careerName, 16) || '—';
  const locationBadge = locationLabel !== 'Sin ubicación'
    ? truncate((currentRoom?.building || student?.current_room_info?.building || student?.current_room?.building) as string | undefined, 18) || locationLabel
    : 'Sin ubicación';

  return (
    <BottomSheet open={isOpen} onClose={handleRequestClose} maxWidthClassName="max-w-md">
      <header className="bg-[var(--color-surface-container-lowest)] border-b border-[var(--color-outline-variant)]/20 p-6 flex flex-col gap-4 relative">
        ...
        <div className="flex items-center gap-4 mt-2">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center font-bold text-xl ring-4 ring-[var(--color-surface-container-lowest)] shadow-sm ${isFemale ? 'bg-pink-100 text-pink-600' : 'bg-[var(--color-primary-light)] text-[var(--color-primary)]'}`}>
            {isLoading ? <div className="w-12 h-12 rounded-2xl bg-[var(--color-surface-container-high)] animate-pulse" /> : initials}
          </div>
          <div>
            <h3 className="text-2xl font-headline font-bold text-[var(--color-primary-dark)] leading-tight">
              {isLoading ? <span className="block w-40 h-6 bg-[var(--color-surface-container-high)] animate-pulse rounded-md"></span> : fullName}
            </h3>
            <p className="text-[var(--color-on-surface-variant)] font-medium mt-1">
              {isLoading ? <span className="block w-24 h-4 bg-[var(--color-surface-container-high)] animate-pulse rounded-md"></span> : `CI: ${student?.ci || '-'}`}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <span className="px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-bold border border-blue-100">
            {isLoading ? <span className="inline-block w-16 h-3 bg-[var(--color-surface-container-high)] animate-pulse rounded"></span> : careerBadge}
          </span>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-[var(--color-surface)] max-h-[70vh]">
        ...
        <section className="soft-lift-card p-5 border border-ghost">
          ...
          <div className="grid grid-cols-2 gap-y-4 gap-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--color-on-surface)] font-semibold">{student?.group_detail?.name || student?.group_name || 'No especificado'}</p>
            </div>
            <div>
              ...
              <p className="text-sm text-[var(--color-on-surface)] font-semibold">
                {currentRoom ? (
                  <span>{currentRoom.number}{currentRoom.wing ? ` · ${currentRoom.wing}` : ''}</span>
                ) : student?.current_room_info ? (
                  <span>{student.current_room_info.room_number}{student.current_room_info.wing ? ` · ${student.current_room_info.wing}` : ''}</span>
                ) : (
                  'No asignado'
                )}
              </p>
